document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchUserScores() {
        try {
            ...
            globalData = {
                currentScore: parseInt(dataOfWhole["classicScoreToday"], 10),
                currentQuestion: parseInt(dataOfWhole["classicQuestionsToday"], 10)
            };
    ...
    async function loadSpecificQuestion() {
        ...
        if (globalData["currentQuestion"]>= 15)
        {
            window.location.href = '/leaderboardClassicSummary';
        }
        else
        {
            window.location.href = '/classicStart';
